...
function M.apply(colors, config)
  ...
  if not ok or not scan then
    vim.notify(
      string.format("[prismpunk] Failed to scan plugin directory: %s", plugin_dir),
      vim.log.levels.WARN
    )
    return
  end
  ...
  while true do
    ...
    if typ == "file" and name:sub(-4) == ".lua" and name ~= "init.lua" then
      ...
      if config.plugins and config.plugins[module_name] == false then
        goto continue
      end
      ...
      if load_ok and type(module.apply) == "function" then
        ...
        vim.notify(
          string.format("[prismpunk] Plugin highlight module invalid: %s", module_path),
          vim.log.levels.WARN
        )
      end
    end
