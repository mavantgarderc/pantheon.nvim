...
M.reload_kitty = function(config_path)
  ...
  vim.fn.system("kitty @ set-colors --all " .. vim.fn.shellescape(config_path) .. " 2>/dev/null")
  if vim.v.shell_error == 0 then return true end
...
M.export_zellij_kdl = function(theme)
  local c = theme.colors
...
M.write_zellij_config = function(theme, config_path)
  local content = M.export_zellij_kdl(theme)
...
M.reload_zellij = function() return true end
...
M.auto_export = function(theme, config)
  ...
  for _, emulator in ipairs(emulators) do
    ...
    if emulator_config and emulator_config.enabled then
      ...
      elseif emulator == "zellij" then
        local success = M.write_zellij_config(theme, emulator_config.config_path)

        if success then
          vim.notify("Prismpunk: Zellij theme exported (restart Zellij session to apply)", vim.log.levels.INFO)
        end
      end
    end
  end
